  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[96vh]">
        ...
        <div id="tct-contract-document" className="relative p-8 sm:p-10 overflow-y-auto space-y-6 flex-1 bg-white text-slate-900 font-sans">
          ...
          <div className="relative flex items-center justify-between border-b-2 border-slate-950 pb-5">
            ...
            <div className="text-right">
              <div className="font-mono text-sm font-black bg-slate-900 text-amber-400 px-3.5 py-1.5 rounded-xl inline-block border border-slate-800 shadow-xs">
          ...
          <div className="space-y-2 text-xs">
            <h3 className="font-black text-slate-900 uppercase tracking-wide flex items-center gap-1.5 text-xs">
              <span className="w-4 h-4 rounded-full bg-slate-900 text-amber-400 text-[10px] flex items-center justify-center font-bold">1</span>
              CLÁUSULA PRIMERA: DE LAS PARTES Y ASESOR RESPONSABLE
            </h3>
            
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-3 leading-relaxed">
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-500 block">EL PRESTADOR (CORPORACIÓN TCT):</span>
                <p className="font-bold text-slate-900">CORPORACIÓN TCT S.A.C.</p>
                <p className="text-slate-600 text-[11px]">RUC: 20608941253</p>
              ...
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-500 block">EL CLIENTE (CONTRATANTE):</span>
                {isEditing ? (
                  <div className="space-y-1.5 pt-1">
                    <input
                      ...
                      className="w-full p-1.5 text-xs font-bold border border-slate-300 rounded-lg"
                  ...
                  <>
                    <p className="font-black text-slate-900 text-sm">{currentData.clientName}</p>
                    <p className="text-slate-600 text-[11px]">
          ...
          <div className="space-y-2 text-xs">
            <h3 className="font-black text-slate-900 uppercase tracking-wide flex items-center gap-1.5 text-xs">
              <span className="w-4 h-4 rounded-full bg-slate-900 text-amber-400 text-[10px] flex items-center justify-center font-bold">2</span>
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <span className="text-[10px] uppercase font-bold text-slate-500 block">Tipo de Evento & Paquete</span>
                <p className="font-black text-slate-900">{currentData.eventType}</p>
                <p className="text-blue-700 font-bold text-[11px]">{currentData.selectedPackageName || 'Paquete Integral TCT'}</p>
              </div>

              <div>
                <span className="text-[10px] uppercase font-bold text-slate-500 block">Fecha y Horario de Cobertura</span>
                <p className="font-black text-slate-900">{currentData.eventDate}</p>
                <p className="text-slate-700 font-bold text-[11px]">{currentData.eventTime} ({currentData.standardHours || 8} horas base)</p>
              </div>

              <div>
                <span className="text-[10px] uppercase font-bold text-slate-500 block">Locación y Dirección Exacta</span>
                <p className="font-bold text-slate-900">{currentData.eventLocation}</p>
                <p className="text-slate-600 text-[11px]">{currentData.eventAddress || 'Lima Metropolitana'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2 text-xs">
            <h3 className="font-black text-slate-900 uppercase tracking-wide flex items-center gap-1.5 text-xs">
              <span className="w-4 h-4 rounded-full bg-slate-900 text-amber-400 text-[10px] flex items-center justify-center font-bold">3</span>
            ...
            <table className="w-full text-xs text-left border border-slate-200 rounded-xl overflow-hidden">
              <thead className="bg-slate-900 text-white text-[10px] uppercase font-black">
                <tr>
                  <th className="p-2.5">Concepto Económico</th>
                  <th className="p-2.5">Detalle / Medios</th>
                  <th className="p-2.5 text-right">Monto (S/.)</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 bg-white">
                <tr>
                  <td className="p-2.5 font-bold">Precio de Lista Base (Paquete Oficial)</td>
                  <td className="p-2.5 text-slate-600">{currentData.selectedPackageName || 'Servicio Audiovisual'}</td>
                  <td className="p-2.5 text-right font-mono font-bold">S/. {(currentData.listPrice || currentData.totalBudget).toLocaleString()}</td>
                </tr>
                {currentData.discountAmount && currentData.discountAmount > 0 ? (
                  <tr className="bg-emerald-50/50">
                    <td className="p-2.5 font-bold text-emerald-900">Descuento Promocional Autorizado</td>
                    <td className="p-2.5 text-emerald-800 text-[11px]">{currentData.discountReason || 'Bonificación Comercial'}</td>
                    <td className="p-2.5 text-right font-mono font-bold text-emerald-700">- S/. {currentData.discountAmount.toLocaleString()}</td>
                  </tr>
                ) : null}
                {currentData.extraHoursCount && currentData.extraHoursCount > 0 ? (
                  <tr>
                    <td className="p-2.5 font-bold text-purple-900">Horas Adicionales ({currentData.extraHoursCount} hrs)</td>
                    <td className="p-2.5 text-slate-600 text-[11px]">Tarifa S/. {currentData.extraHourRate || 150} / hr</td>
                    <td className="p-2.5 text-right font-mono font-bold text-purple-900">+ S/. {totalExtraHours.toLocaleString()}</td>
                  </tr>
                ) : null}
                <tr className="bg-slate-100 font-black">
                  <td className="p-2.5 text-slate-900">PRESUPUESTO TOTAL PACTADO:</td>
                  <td className="p-2.5 text-slate-600 text-[11px]">Moneda: Soles Peruanos (PEN)</td>
                  <td className="p-2.5 text-right font-black font-mono text-sm text-slate-950">S/. {computedTotal.toLocaleString()}</td>
                </tr>
                <tr>
                  <td className="p-2.5 font-bold text-slate-900">1. Adelanto Inicial (Firma / Reserva)</td>
                  <td className="p-2.5 text-slate-600">{currentData.paymentMethodDeposit || 'Transferencia Bancaria / Efectivo'}</td>
                  <td className="p-2.5 text-right font-mono font-bold text-emerald-700">S/. {currentData.initialDeposit.toLocaleString()}</td>
                </tr>
                <tr className="bg-amber-50/60 font-bold">
                  <td className="p-2.5 text-amber-950">2. Saldo en Campo (Límite 7:00 PM)</td>
                  <td className="p-2.5 text-amber-900 text-[11px]">
                    * Obligatorio liquidar en locación antes de 7:00 PM para habilitar postproducción
                  </td>
                  <td className="p-2.5 text-right font-mono font-black text-amber-950">S/. {balanceRemaining.toLocaleString()}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="space-y-2 text-xs">
            <h3 className="font-black text-slate-900 uppercase tracking-wide flex items-center gap-1.5 text-xs">
              <span className="w-4 h-4 rounded-full bg-slate-900 text-amber-400 text-[10px] flex items-center justify-center font-bold">4</span>
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                <div className="p-2.5 bg-white rounded-xl border border-slate-200">
                  ...
                  <span className="text-[11px] text-purple-700 font-bold">Plazo: 15 días hábiles</span>
                  <p className="text-[10px] text-slate-500 mt-0.5">Enlace digital y resguardo en Servidor</p>
                </div>

                <div className="p-2.5 bg-white rounded-xl border border-slate-200">
                  <span className="font-bold text-slate-900 block">📖 Fotolibro Impreso</span>
                  <span className="text-[11px] text-pink-700 font-bold">{currentData.includesPhotobook ? 'Plazo: 30 días hábiles' : 'No incluido en paquete'}</span>
                  <p className="text-[10px] text-slate-500 mt-0.5">Maquetación, aprobación y encuadernado</p>
                </div>

                <div className="p-2.5 bg-white rounded-xl border border-slate-200">
                  <span className="font-bold text-slate-900 block">💾 Estuche de Madera & USB 3.0</span>
                  <span className="text-[11px] text-emerald-700 font-bold">Entrega con Saldo S/. 0</span>
                  <p className="text-[10px] text-slate-500 mt-0.5">Material final en alta definición 4K</p>
                </div>
              </div>

              <p className="text-[10px] text-slate-600 italic">
                * Conforme al Paso 12 del flujo oficial, CORPORACIÓN TCT conservará los archivos RAW originales por un periodo máximo de 30 días posteriores a la entrega final, tras el cual se procederá a la depuración de servidores.
              </p>
            </div>
          </div>

          {/* Official Signatures */}
          <div className="pt-8 border-t-2 border-slate-950 grid grid-cols-2 gap-10 text-center text-xs">
            ...
            <div className="space-y-12">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500">DNI / RUC: {currentData.clientDniRuc || '__________________'}</p>
