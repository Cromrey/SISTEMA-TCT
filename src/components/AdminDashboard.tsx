...
import { ProductionProject, SmartAlert, DecisionInsight, EventType, StepData, StaffMember, AuthUser } from '../types';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const effectiveUsersList = allUsers && allUsers.length > 0 ? allUsers : getStoredUsers();
  ...
  return (
    <div className="space-y-6">
      ...
      <div id="tct-projects-list-section" className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        ...
        {/* Barra Interactiva de Navegación por Asesores / Usuarios Creados */}
        <div className="p-2.5 sm:p-3 bg-slate-50 rounded-2xl border border-slate-200 shadow-2xs space-y-2">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-1.5 text-xs font-black text-slate-800 uppercase tracking-wide">
              <Users className="w-4 h-4 text-amber-600" />
              <span>Navegación por Asesor / Usuario:</span>
            </div>

            {onOpenUsersManagement && (
              <button
                onClick={onOpenUsersManagement}
                className="px-2.5 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black flex items-center gap-1 shadow-xs transition-all cursor-pointer"
                title="Abrir ventana de creación y gestión de usuarios"
              >
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {/* Todos los Usuarios tab */}
            <button
              onClick={() => setSelectedUserFilter('all')}
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
                !selectedUserFilter || selectedUserFilter === 'all'
                  ? 'bg-slate-900 text-amber-400 font-black shadow-xs ring-1 ring-slate-950'
                  : 'bg-white text-slate-700 hover:bg-slate-200 border border-slate-200'
              }`}
            >
              <span>Todos los Usuarios</span>
              <span className="text-[10px] px-1.5 py-0.2 rounded-full font-mono bg-slate-800 text-slate-200 font-bold">
                {projects.length}
              </span>
            </button>
            ...
            {effectiveUsersList.map((usr) => {
              ...
              return (
                <button
                  key={usr.id}
                  onClick={() => setSelectedUserFilter(isSelected ? 'all' : usr.id)}
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
                    isSelected
                      ? 'bg-amber-500 text-slate-950 font-black shadow-xs ring-2 ring-amber-400'
                      : 'bg-white text-slate-700 hover:bg-slate-200 border border-slate-200'
                  }`}
                  title={`Filtrar por ${usr.fullName} (${usr.role === 'admin' ? 'Administrador' : 'Asesor'})`}
                >
                  <div className={`w-4 h-4 rounded-full text-[9px] font-black flex items-center justify-center ${
                    isSelected ? 'bg-slate-950 text-amber-400' : 'bg-slate-800 text-white'
                  }`}>
                    {usr.username.charAt(0).toUpperCase()}
                  </div>
                  <span>{usr.fullName.split(' ')[0]} ({usr.username})</span>
